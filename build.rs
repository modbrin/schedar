...
fn compile_to_spirv(name: &str) {
    ...
    let out = Command::new("glslc").args([vert_src.as_str(), "-o", vert_out.as_str()]).output().expect(&format!("failed to compile vertex shader: {}", name));
    if !out.status.success() {
        panic!("{}", String::from_utf8(out.stderr).unwrap());
    }
    let out = Command::new("glslc").args([frag_src.as_str(), "-o", frag_out.as_str()]).output().expect(&format!("failed to compile fragment shader: {}", name));
    if !out.status.success() {
        panic!("{}", String::from_utf8(out.stderr).unwrap());
    }
